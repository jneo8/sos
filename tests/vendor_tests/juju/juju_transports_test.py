...
@dataclass
class MockCommons:
    cmdlineopts: MockCmdLineOpts = field(default=MockCmdLineOpts())
...
class JujuSSHTest(unittest.TestCase):
    ...
    def test_check_juju_installed_err(self, mock_subprocess_check_output):
        ...
        mock_subprocess_check_output.side_effect = subprocess.CalledProcessError(
            returncode="127", cmd="cmd_abc"
        )
        with self.assertRaises(JujuNotInstalledException) as context:
    ...
    def test_chmod(self, mock_subprocess_check_output):
        result = self.juju_ssh._chmod(fname="file_abc")
        mock_subprocess_check_output.assert_called_with(
            f"{self.juju_ssh.remote_exec} chmod o+r file_abc",
            stderr=subprocess.STDOUT,
            shell=True,
        )

    @patch(
        "sos.collector.transports.juju.JujuSSH._check_juju_installed", return_value=True
    )
    ...
    def test_remote_exec(self):
        assert self.juju_ssh.remote_exec == "juju ssh -m model_abc unit_abc sudo"
